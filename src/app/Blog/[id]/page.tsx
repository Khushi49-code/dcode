...
export default function BlogDetailPage() {
  ...
  const formatContent = (content: string) => {
    ...
    lines.forEach((line, index) => {
      ...
      // Check for H2 headings (lines that start with ## or are ALL CAPS and short)
      if (trimmedLine.startsWith('## ') || 
          (trimmedLine === trimmedLine.toUpperCase() && trimmedLine.length < 60 && trimmedLine.length > 5 && !trimmedLine.includes('.'))) {
        const headingText = trimmedLine.startsWith('## ') ? trimmedLine.replace('## ', '') : trimmedLine;
        formattedLines.push(
          <h2 key={`h2-${index}`} className="text-2xl font-bold text-white mt-14 mb-4 tracking-wide">
            {headingText}
          </h2>
        );
        return;
      ...
      // Check for H3 headings (lines that start with ###)
      if (trimmedLine.startsWith('### ')) {
        formattedLines.push(
          <h3 key={`h3-${index}`} className="text-xl font-semibold text-white mt-10 mb-3 tracking-wide">
            {trimmedLine.replace('### ', '')}
          </h3>
        );
        return;
      ...
      if (trimmedLine.startsWith('- ') || trimmedLine.startsWith('• ')) {
        ...
        listItems.push(
          <li key={`li-${index}`} className="text-lg leading-8 text-gray-300 ml-6 mb-2 list-disc">
            {listText}
          </li>
        );
        return;
      ...
      if (isList && !trimmedLine.startsWith('- ') && !trimmedLine.startsWith('• ')) {
        formattedLines.push(
          <ul key={`ul-${index}`} className="space-y-1 my-6 pl-2">
            {listItems}
          </ul>
        );
        isList = false;
      ...
      // Check for bold text (lines wrapped in ** or __)
      let processedLine = trimmedLine;
      if (processedLine.includes('**') || processedLine.includes('__')) {
      ...
      if (trimmedLine) {
        formattedLines.push(
          <p 
            key={`p-${index}`} 
            className="text-lg leading-8 text-gray-300 mb-5"
            dangerouslySetInnerHTML={{ __html: processedLine }}
          />
        );
      } else {
        // Empty line - add spacing
        formattedLines.push(<div key={`spacer-${index}`} className="h-3"></div>);
      }
    });
    ...
    if (isList) {
      formattedLines.push(
        <ul key="ul-final" className="space-y-1 my-6 pl-2">
          {listItems}
        </ul>
      );
    }
      ...
      <div className="min-h-screen bg-black text-white">
        ...
        <div className="px-6 pb-12 max-w-4xl mx-auto">
          ...
          <article className="prose prose-lg prose-invert max-w-none" itemProp="articleBody">
            <div className="text-gray-300 leading-relaxed font-normal">
